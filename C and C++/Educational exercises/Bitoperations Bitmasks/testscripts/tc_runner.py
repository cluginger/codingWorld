...
class tc_runner:
    ...
    def run_as_full_io(self,cmd):
        ...
        self.result["diff"]= []
        self.result["retcode"] = -99
        if "return" in self.desc:
            self.result["expected_retcode"] = self.desc["return"]
        outp = ""
        ...
        try:
            ...
            child.close()
            self.result["retcode"] = retcode_conversion(child.exitstatus)
            ...
            if(lev == 0):
                ...
                if("return" in self.desc):
                    if(self.result["retcode"] != self.desc["return"]):
                        self.result["passed"] =False
...
def retcode_conversion(retcode):
    if retcode > 127 :
        return -(256-retcode)
    else:
        return retcode
