...
def detail_report(tc,name,parent,tc_def = None):
    ...
    if "return" in tc_def and "retcode" in tc:
        retvar_panel = ET.SubElement(card,"div")
        if tc_def["return"]==tc["retcode"]:
            retvar_panel.attrib["class"] = "w3-panel w3-light-green w3-serif"
        else:
            retvar_panel.attrib["class"] = "w3-panel w3-deep-orange w3-serif"
        rvar = ET.SubElement(retvar_panel,"p")
        rvar.text = "Return Value was {}, should be {} !".format(str(tc["retcode"]),str(tc_def["return"]))
