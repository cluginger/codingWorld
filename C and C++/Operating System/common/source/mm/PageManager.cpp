...
uint32 PageManager::allocPPN(uint32 page_size)
{
  ...
  if (found == 0)
  {
    ...
    debug(SWAP, "PageManager::allocPPN: Out of memory / No more free\n");
    size_t ppn = SwapManager::instance()->addSwapOutRequest();
    debug(SWAP, "PageManager allocppn -> we just swapped out page: %p\n", (void*)ppn);
    debug(USED_PPNs, "allocPPn ppn %p is now allocated by Thread %ld\n", ppn, currentThread->getTID());
    return ppn;
  ...
  debug(USED_PPNs, "allocPPN %p is now allocated by Thread %lu\n", found, currentThread->getTID());
  return found;
}

void PageManager::freePPN(uint32 page_number, uint32 page_size)
{
//  debug(ARCHMEM, "THREAD %ld freeing %zx\n",currentThread->getTID(), page_number);

  debug(USED_PPNs, "[Free PPN] on ppn %p\n", page_number);
  assert((page_size % PAGE_SIZE) == 0);
...
void PageManager::printUsedPPNs()
{
  lock_.acquire();
  for(uint32 ppn = 0; ppn < number_of_pages_; ppn++)
  {
    if(page_usage_table_->getBit(ppn))
    {
      if(ppn >= 0x400)
      {
        debug(USED_PPNs, "ppn : %p is used\n", ppn);
...
bool PageManager::checkIfPageCanBeFreed(size_t ppn)
{
  if(!page_usage_table_->getBit(ppn))
  {
    return false;
