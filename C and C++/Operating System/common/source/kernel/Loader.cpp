...
Loader::Loader(Loader &new_loader, ssize_t fd) : arch_memory_(new_loader.arch_memory_),fd_(fd), hdr_(0), phdrs_(), program_binary_lock_("Loader::program_binary_lock_"), userspace_debug_info_(0)
{
}
...
void Loader::loadPage(pointer virtual_address)
{
  ...
  debug(SWAP, "ALLOCING in LOADPAGE %p\n", (void*)ppn);
  ...
  {
    PageManager::instance()->freePPN(ppn);
    debug(LOADER, "error occurs at ppn %p", ppn);
    debug(LOADER, "Loader::loadPage: ERROR! No section refers to the given address.\n");
    Syscall::exit(666);
  }
  debug(USED_PPNs, "Prealloc in loadPage for address %p\n", virtual_address);
  PreAllocate pages(3);
  IPT::instance()->ipt_lock.acquire();
  arch_memory_.arch_memory_lock.acquire();
  bool page_mapped = arch_memory_.mapPage(virt_page_start_addr / PAGE_SIZE, ppn, true, &pages);
  if (!page_mapped)
  {
  ...
  arch_memory_.arch_memory_lock.release();
...
size_t Loader::getHeapStart()
{
  size_t heapStart = 0;
  size_t errorRetval = 0;
  //we go through each header, and we want to take the last one!
  program_binary_lock_.acquire();
  if(sizeof(Elf::Phdr) != hdr_->e_phentsize)
  {
    debug(LOADER, "Expected program header size does not match advertised program header size\n");
  ...
  if(!prepareHeaders()) //sorts headers
  {
    debug(LOADER, "Loader::readHeaders: ERROR! There are no valid sections in the binary.\n");
    program_binary_lock_.release();
    return errorRetval;
  ...
  {
    size_t newHighestHeader = phdrs_[i].p_vaddr + phdrs_[i].p_memsz;
//    debug(USERTHREAD, "new highest header %p\n", newHighestHeader);
    if(newHighestHeader > heapStart)
    {
      heapStart = newHighestHeader;
    }
  }
  heapStart += PAGE_SIZE;
  program_binary_lock_.release();
