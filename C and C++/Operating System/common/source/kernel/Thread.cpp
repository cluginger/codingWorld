    ...
    my_terminal_(0), working_dir_(working_dir), name_(ustl::move(name))
{
  ...
  ArchThreads::atomic_set(tid_,ArchThreads::atomic_add(currentTID,1));
...
{
  bool noMoreAsleep = Scheduler::instance()->getRdtscTicks() > asleep;
  bool isRunning = (getState() == Running);
  return (noMoreAsleep && isRunning);
}
...
void Thread::setName(char *filename)
{
  this->name_ = (ustl::string)filename;
}
