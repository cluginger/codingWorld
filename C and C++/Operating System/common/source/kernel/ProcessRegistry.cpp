...
ProcessRegistry::ProcessRegistry(FileSystemInfo *root_fs_info, char const *progs[]) :
        ...
        waitpid_cond(&waitpid_lock, "waitpid_cond"),
        waitpid_lock("waitpid_lock"),
        PID_lock("PID_lock"),
        processMap_lock("processMap_lock"),
        deadProcessMap_lock("deadProcessMap_lock"),
        aslrModeLock("aslrModeLock"),
        progs_(progs),
        progs_running_(0),
...
void ProcessRegistry::createProcess(const char* path) {
  ...
  while(Scheduler::instance()->getTicks() < 2)
  {
    Scheduler::instance()->yield();
  }
  auto process = new UserProcess(path, new FileSystemInfo(*working_dir_));
  processMap_lock.acquire();
  processMap.insert(ustl::make_pair<size_t, UserProcess*>(process->getPID(), process));
  processMap_lock.release();
  debug(PROCESS_REG, "created userprocess %p\n", process);
}

size_t ProcessRegistry::getAndEnhancePIDCounter()
{

  PID_lock.acquire();
  auto pid = pid_counter++;
  PID_lock.release();

  return pid;
}

void ProcessRegistry::switchASLRStatus()
{

  aslrModeLock.acquire();
  bool currentaslr = isAslrModeOn;
  if(currentaslr)
  {
    isAslrModeOn = false;
    kprintf("ASLR switched OFF\n");
  }
  else
  {
    isAslrModeOn = true;
    kprintf("ASLR switched ON\n");
  }
  aslrModeLock.release();
  kprintf("SWEB: /> ");
}