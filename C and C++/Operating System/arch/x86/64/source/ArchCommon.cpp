...
#define STATS_OFFSET 2
#define FREE_PAGES_OFFSET STATS_OFFSET + 11*2

void ArchCommon::drawStat() {
//  const char* text  = "Free pages      F9 MemInfo   F10 Locks   F11 Stacktrace   F12 Threads"; //oldest
//  const char* text  = "Free pages     F8 PRA  F9 MemInfo  F10 Locks  F11 Stacktrace  F12 Threads"; //2nd version
//    const char* text  = "Free pages     F7 PRA INFO F8 switch PRA F9 MemInfo F10Locks  F11ST F12T";
                                                                                                   //F3 IPT F4 SPT
    const char* text  = "Free pages     F3 IPT F4 SPT F5 DiskBitmap F6 Aslr F7 PRA Info F8 PRA Switch                   F9 Mem F10 Locks F11 Trace F12 Threads"; //3rd version
    const char* green = "xxxxxxxxxx                                                                                     xx     xxx       xxx       xxx  ";
    const char* blue =  "                                                   xx          xx                                                         ";
    const char* red =   "                             xx            xx                                                                              ";
    const char* yellow ="               xx     xx                                                                              ";
//                                      15            29      37          49
    ...
    while(text[i])
    {
        ...
        if(i > 49 && i < 65)
          fb[i * 2 + STATS_OFFSET + 1] = (char)(blue[i] == 'x' ? 0x30 : 0x08);
        else if(i > 24 && i < 49)
          fb[i * 2 + STATS_OFFSET + 1] = (char)(red[i] == 'x' ? 0x40 : 0x08);
        else if(i > 10 && i < 24)
          fb[i * 2 + STATS_OFFSET + 1] = (char)(yellow[i] == 'x' ? 0x60 : 0x08);
        else
          fb[i * 2 + STATS_OFFSET + 1] = (char)(green[i] == 'x' ? 0x20 : 0x08);
        i++;
    }
