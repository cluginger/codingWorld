...
class ArchMemory
{
  public:
    ...
    void handleCow(size_t address);
    bool isDirty(size_t vpn, size_t ppn);
    ssize_t getOffset(size_t vpn);
    void handleHeap(size_t address);
    void handleAccessedBits(size_t pml4_);
    void handleBrk(size_t vpn);
    size_t countPresentBits(PageMapLevel4Entry* pml4);
    Mutex arch_memory_lock;
     ...
     */
    [[nodiscard]] bool mapPage(uint64 virtual_page, uint64 physical_page, uint64 user_access, PreAllocate *ppns);
