...
# enter your code here
    # random nonce (4)
    next_nonce = getRandomNonce()

    first_xor_state = []

    for a in range(0, LFSR_LEN):
        first_xor_state.append(bytesToBits(cts[0])[a]^bytesToBits(cts[1])[a])



    xor_lfsr_stream = lfsr.LFSR(LFSR_LEN, first_xor_state, LFSR_TAPS)
    xor_lfsr_stream.output(LFSR_LEN)

    second_xor_state = []

    for y in range(0, LFSR_LEN):
        second_xor_state.append(bytesToBits(cts[1])[y] ^ xor_lfsr_stream.state[y])



    next_ct = bitsToBytes(second_xor_state)
